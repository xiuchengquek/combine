from behave import *
from functions.bin_example import extract_exon
...
def step_impl(context):
    context.output = "\n".join(context.output)
    assert context.output, context.text

@then(u'Their average exon length are')
def step_impl(context):
    mock_lengths =  {}
    for rows in context.table :
        mock_lengths[rows['gene']] = rows['average_exon_len']
    assert mock_lengths, context.output
